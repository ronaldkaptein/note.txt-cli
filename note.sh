...
DESCRIPTION
   ...
   OPTIONS:
    ...
    -p PREFIX
            Prefix to use before title  (default is none). Accepts bash date sequences
    ...
    -y NAME
            Name for today note. Default is %Y%m%d.
   ...
   ACTIONS:
    ...
    today|t
      opens today note. Note title is %Y%m%d by default, but can be customized using -y option.
    help|h
      Displays this help
...
function open(){
  ...
  if [ "$1" == "today" ]; then
    File=$(date +$TodayNoteName)$Extension
  elif [ "$1" == "last" -o "$1" == "l" ]; then
...
while getopts "afhmd:l:qp:e:g:s:tijonrk:uy:" OPTION
do
  case $OPTION in
    h)
      usage
